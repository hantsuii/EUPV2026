...
const aspRegionSel = byId("aspRegionSel"), aspFamilySel = byId("aspFamilySel");
const productMetricSel = byId("productMetricSel"), productDimensionSel = byId("productDimensionSel"), productIncomeSel = byId("productIncomeSel");
...
const I18N = {
  zh: {
    ...
    filterIncome: "收入类型", filterYear: "年份", filterQuarter: "季度", filterMonth: "月份", filterRegion: "地区", filterProductFamily: "Product Family", incomeTotal: "总计", incomeInvoiced: "开票收入", incomeConfirm: "待确认收入", all: "全部",
    ...
    chartAmountTitle: "2026 月度销售金额变化（折线=总金额，柱=组件/ESS）", chartAmountY: "金额(万€)", chartTotalAmount: "总金额(万€)", chartPvAmount: "组件金额(万€)", chartEssAmount: "ESS金额(万€)", aspFilterTitle: "ASP 筛选", chartPvAspTitle: "2026 月度组件ASP变化", chartPvAsp: "组件ASP(€/W)", chartEssAspTitle: "2026 月度储能ASP变化", chartEssAsp: "储能ASP(€/Set)",
    regTotal: "总金额(万€)", regBp: "BP金额(万€)", regBpRate: "BP达成（开票/总计）", regShare: "总金额占比", regEssAmount: "储能金额(万€)", regEssQty: "储能销量(Sets)", regEssAsp: "储能ASP(€/Set)", regPvAmount: "组件金额(万€)", regPvQty: "组件销量(MW)", regPvAsp: "组件ASP(€/W)",
    productTitle: "产品销售分析", productMetric: "分析指标", productDimension: "产品维度", metricRevenue: "销售收入", metricPvQty: "PV 销量 (MW)", metricEssQty: "ESS 销量 (Sets)", startMonth: "开始月份", endMonth: "结束月份", productNote: "数量视图分别显示 PV MW 或 ESS Sets，避免不同单位相加。", productTotal: "筛选区间总计", productTop: "领先产品", productTopShare: "领先产品占比", productMom: "最近月环比", productTrend: "月度趋势", productShare: "销售占比", rankName: "产品", rankValue: "指标值", rankShare: "占比", rankLatest: "最近月", rankPrevious: "上月", rankMom: "环比", noData: "无数据", other: "其他", empty: "",
  },
  en: {
    ...
    filterIncome: "Income Type", filterYear: "Year", filterQuarter: "Quarter", filterMonth: "Month", filterRegion: "Region", filterProductFamily: "Product Family", incomeTotal: "Total", incomeInvoiced: "Invoiced", incomeConfirm: "Confirm", all: "All",
    ...
    chartAmountTitle: "2026 Monthly Sales Amount (line=total, bars=PV/ESS)", chartAmountY: "Amount (10k €)", chartTotalAmount: "Total Amount (10k €)", chartPvAmount: "PV Amount (10k €)", chartEssAmount: "ESS Amount (10k €)", aspFilterTitle: "ASP Filters", chartPvAspTitle: "2026 Monthly PV ASP", chartPvAsp: "PV ASP (€/W)", chartEssAspTitle: "2026 Monthly ESS ASP", chartEssAsp: "ESS ASP (€/Set)",
    regTotal: "Total Amount (10k €)", regBp: "BP Revenue (10k €)", regBpRate: "BP Achievement (Inv./Total)", regShare: "Share", regEssAmount: "ESS Amount (10k €)", regEssQty: "ESS Qty (Sets)", regEssAsp: "ESS ASP (€/Set)", regPvAmount: "PV Amount (10k €)", regPvQty: "PV Qty (MW)", regPvAsp: "PV ASP (€/W)",
    productTitle: "Product Sales Analysis", productMetric: "Metric", productDimension: "Product Dimension", metricRevenue: "Sales Revenue", metricPvQty: "PV Qty (MW)", metricEssQty: "ESS Qty (Sets)", startMonth: "Start Month", endMonth: "End Month", productNote: "PV MW and ESS Sets are shown separately so unlike units are never added.", productTotal: "Period Total", productTop: "Leading Product", productTopShare: "Leading Product Share", productMom: "Latest MoM", productTrend: "Monthly Trend", productShare: "Sales Share", rankName: "Product", rankValue: "Value", rankShare: "Share", rankLatest: "Latest Month", rankPrevious: "Previous Month", rankMom: "MoM", noData: "No data", other: "Other", empty: "",
  },
};
...
function normalizeTargets(rawRows) {
  return rawRows.map((row) => {
    ...
    return { year, month, quarter: normalizeQuarter(row["Quartely"], month), region: mapRegionForStats(row["Region"] ?? row["Country"]), category, revenue: n(row["Revenue EUR"]), quantity, pvQty: category === "PV" ? quantity : 0, essQty: category === "ESS" ? quantity : 0 };
...
function initFilters() {
  const allOption = { value: "__ALL__", label: t("all") }, years = [...new Set(allRows.map((r) => r.year))].sort((a, b) => a - b), months = [...new Set(allRows.map((r) => r.month))].sort(), regions = [...new Set(allRows.map((r) => r.region))].sort(), families = [...new Set(allRows.map((r) => r.productFamily))].sort();
  ...
  fillSelect(aspRegionSel, [allOption, ...regions.map((v) => ({ value: v, label: v }))]); fillSelect(aspFamilySel, [allOption, ...families.map((v) => ({ value: v, label: v }))]); fillSelect(productRegionSel, [allOption, ...regions.map((v) => ({ value: v, label: v }))]);
...
function renderOverview() {
  const yearRows = allRows.filter((r) => r.year === TARGET_YEAR), targetYear = allTargets.filter((r) => r.year === TARGET_YEAR);
  ...
  const future = aggregate(allRows.filter((r) => r.year > TARGET_YEAR)); byId("futureKpiRow").innerHTML = [card(t("kpiFutureRevenue"), fmtWanInt(future.revenue)), card(t("kpiFuturePvQty"), fmtInt(future.pvQty)), card(t("kpiFutureEssQty"), fmtInt(future.essQty))].join("");
  ...
  byId("h1KpiRow").innerHTML = [card(t("kpiH1Revenue"), fmtWanInt(h1Inv.revenue)), card(t("kpiH1PvQty"), fmtInt(h1Inv.pvQty)), card(t("kpiH1EssQty"), fmtInt(h1Inv.essQty)), card(t("kpiH1Rate"), fmtPct(achievement(h1Inv.revenue, h1Bp.revenue)))].join("");
  renderQuarterProgress(yearRows, targetYear); renderMonthlyRevenue(yearRows); renderAspCharts(yearRows); setStatus(t("statusDone", { allRows: allRows.length, targetRows: allTargets.length }));
}
function renderQuarterProgress(yearRows, targetYear) {
  ...
  table("quarterProgressTable", [t("qPeriod"), t("qInvoicedAmount"), t("qConfirmAmount"), t("qAllAmount"), t("qTargetAmount"), t("qInvRate"), t("qAllRate"), t("qPvQty"), t("qEssQty")], rows);
}
function renderMonthlyRevenue(yearRows) {
  ...
  renderPlot("monthlyTrendChart", [{ x: months, y: data.map((x) => x.revenue / 10000), type: "scatter", mode: "lines+markers", name: t("chartTotalAmount"), line: { color: "#458CFF", width: 3 } }, { x: months, y: data.map((x) => x.pvAmount / 10000), type: "bar", name: t("chartPvAmount"), marker: { color: "#7CB4FF" } }, { x: months, y: data.map((x) => x.essAmount / 10000), type: "bar", name: t("chartEssAmount"), marker: { color: "#B48BFF" } }], { title: t("chartAmountTitle"), barmode: "group", yaxis: { title: t("chartAmountY") } });
}
function renderAspCharts(yearRows = allRows.filter((r) => r.year === TARGET_YEAR)) {
  let rows = filterMulti(yearRows, selected(aspRegionSel), (r) => r.region); rows = filterMulti(rows, selected(aspFamilySel), (r) => r.productFamily);
  const months = Array.from({ length: 12 }, (_, i) => `${TARGET_YEAR}-${String(i + 1).padStart(2, "0")}`), data = months.map((m) => aggregate(rows.filter((r) => r.month === m))), pv = data.map((x) => x.pvQty !== 0 ? x.pvAmount / (x.pvQty * 1000000) : null), ess = data.map((x) => x.essQty !== 0 ? x.essAmount / x.essQty : null);
  renderPlot("monthlyPvAspChart", [{ x: months, y: pv, type: "scatter", mode: "lines+markers+text", text: pv.map((v) => v == null ? "" : fmtAsp3(v)), textposition: "top center", name: t("chartPvAsp"), line: { color: "#2E7CFF", width: 3 } }], { title: t("chartPvAspTitle"), yaxis: { title: t("chartPvAsp") } });
  renderPlot("monthlyEssAspChart", [{ x: months, y: ess, type: "scatter", mode: "lines+markers+text", text: ess.map((v) => v == null ? "" : fmtInt(v)), textposition: "top center", name: t("chartEssAsp"), line: { color: "#9D63FF", width: 3 } }], { title: t("chartEssAspTitle"), yaxis: { title: t("chartEssAsp") } });
}
...
function renderProduct() {
  ...
  const metric = productMetricSel.value, dimension = productDimensionSel.value;
  let start = productStartMonthSel.value, end = productEndMonthSel.value;
  if (start > end) [start, end] = [end, start];
  let rows = filterIncome([...allRows], productIncomeSel.value).filter((r) => r.month >= start && r.month <= end); if (productRegionSel.value !== "__ALL__") rows = rows.filter((r) => r.region === productRegionSel.value); if (metric === "pvQty") rows = rows.filter((r) => r.isPV); if (metric === "essQty") rows = rows.filter((r) => r.isESS);
  ...
  renderPlot("productShareChart", [{ labels: pieData.map((x) => x[0]), values: pieData.map((x) => x[1]), type: "pie", hole: 0.42, textinfo: "label+percent" }], { title: `${t("productShare")} · ${metricTitle}` });
...
[incomeTypeSel, yearSel, quarterSel, monthSel].forEach((el) => el.addEventListener("change", renderRegion)); [aspRegionSel, aspFamilySel].forEach((el) => el.addEventListener("change", () => renderAspCharts())); [productMetricSel, productDimensionSel, productIncomeSel, productRegionSel, productStartMonthSel, productEndMonthSel].forEach((el) => el.addEventListener("change", renderProduct)); [incomeTypeSel, yearSel, quarterSel, monthSel, aspRegionSel, aspFamilySel].forEach(bindMultiSelectToggle);
