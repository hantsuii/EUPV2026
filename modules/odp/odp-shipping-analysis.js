...
function arrivalInfo(record){if(record.ata)return{date:record.ata,type:"actual"};if(record.etaUpdate)return{date:record.etaUpdate,type:"forecast"};if(record.etaSO)return{date:record.etaSO,type:"forecast"};if(record.odpEtaPort)return{date:record.odpEtaPort,type:"planned"};return{date:null,type:"missing"};}
function isArrivalValid(record){if(record.poType==="OFFSHORE")return false;const a=arrivalInfo(record);return a.type!=="missing";}
...
function setDefaultMonthRanges(){const now=new Date(),current=monthKey(now),nextMonth=new Date(now.getFullYear(),now.getMonth()+1,1),next=monthKey(nextMonth);["order","departure"].forEach((scope)=>{byId(`${scope}StartMonth`).value=current;byId(`${scope}EndMonth`).value=current;});byId("arrivalStartMonth").value=current;byId("arrivalEndMonth").value=next;}
...
function activeProductType(){return byId("productTypeSelect")?.value||"PV";}
function activeSources(){const pt=activeProductType();if(pt==="ALL")return new Set(["PV SUPPLY DATA","ESS SUPPLY DATA"]);if(pt==="ESS")return new Set(["ESS SUPPLY DATA"]);return new Set(["PV SUPPLY DATA"]);}
function activeMainRecords(){const sources=activeSources();return records.filter((r)=>sources.has(r.source));}
/* Independent product type filters for arrival and portDaily panels */
const panelProductTypes={arrival:"PV",portDaily:"PV"};
function panelSources(panel){const pt=panelProductTypes[panel];if(pt==="ALL")return new Set(["PV SUPPLY DATA","ESS SUPPLY DATA"]);if(pt==="ESS")return new Set(["ESS SUPPLY DATA"]);return new Set(["PV SUPPLY DATA"]);}
function panelMainRecords(panel){const sources=panelSources(panel);return records.filter((r)=>sources.has(r.source));}
function availableModels(){
  const models=new Map();
  activeMainRecords().forEach((r)=>{const key=modelKey(r);if(!models.has(key))models.set(key,modelLabel(r));});
...
function availablePortOptions(){
  const ports=new Set();
  panelMainRecords("portDaily").forEach((r)=>{if(!isArrivalValid(r))return;const a=arrivalInfo(r);if(a.date){const dest=resolvePort(r.rawDestination,"DEST");if(dest)ports.add(dest);}});
  return[...ports].sort((a,b)=>a.localeCompare(b)).map((p)=>({key:p,label:p}));
}
function commitPortFilter(root){
  if(!root)return;
  const query=normalizeText(root.querySelector('.model-filter-search')?.value),allBoxes=[...root.querySelectorAll('.model-filter-options input[type="checkbox"]')];
  const boxes=query?allBoxes.filter((x)=>!x.closest('.model-filter-option')?.hidden):allBoxes;
  const chosen=new Set((query?boxes:boxes.filter((x)=>x.checked)).map((x)=>x.value));
  portFilterSelection=!allBoxes.length||chosen.size===allBoxes.length?null:chosen;
}
function renderPortFilter(){
  const root=document.querySelector(".port-filter");
  if(!root)return;
  const options=availablePortOptions();
...
function readSheet(workbook,sheetName,headerRow,productType){
  ...
  return grid.slice(headerRow).flatMap((row)=>{
    ...
    let rawPoType=index["New Ark PO Type"]!=null?clean(row[index["New Ark PO Type"]]):null;
    return[{source:sheetName,productType:productType||"PV",poType:rawPoType?String(rawPoType).trim().toUpperCase():"",reference:String(ref).trim(),quantity:toNumber(row[index.QUANTITY]),mw:toNumber(row[index.MW])||0,containers:toNumber(row[index.CONTAINERS])||0,sku:index["New Ark SKU"]!=null?clean(row[index["New Ark SKU"]]):index["PN"]!=null?clean(row[index.PN]):null,model:index.Model!=null?clean(row[index.Model]):index.DESCRIPTION!=null?clean(row[index.DESCRIPTION]):null,factory:index["Factory Location"]!=null?clean(row[index["Factory Location"]]):null,carrier:index.Carrier!=null?clean(row[index.Carrier]):null,status:index.STATUS!=null?String(clean(row[index.STATUS])||""):"",pickupDate:index["Pick-Up Date"]!=null?toDate(row[index["Pick-Up Date"]]):index["PICK-UP DATE"]!=null?toDate(row[index["PICK-UP DATE"]]):null,pickupWeek:index["Pick-Up Week"]!=null?row[index["Pick-Up Week"]]:null,rawPol,rawDestination:clean(row[index["PORT DESTINATION"]]),etdSO:toDate(row[index["ETD On S/O"]]),etdUpdate:toDate(row[index["ETD Update"]]),atd:toDate(row[index["ATD PORT"]]),etaSO:toDate(row[index["ETA On S/O"]]),etaUpdate:toDate(row[index["ETA Update"]]),ata:toDate(row[index["ATA PORT"]]),odpEtaPort:toDate(row[index["ODP ETA Port"]]),vessel:index["REFERENCE V.V"]!=null?clean(row[index["REFERENCE V.V"]]):null,booking:index[bookingField]!=null?clean(row[index[bookingField]]):null}];
  });
}
...
function parseWorkbook(workbook,skuModelMap){
  const pvSources=[["PV SUPPLY DATA",1],["H2-2025 PV DATA",1],["H1-2025 PV DATA",3]],essSources=[["ESS SUPPLY DATA",1]],unique=new Map();quantityZeroCount=0;invalidQuantityCount=0;
  pvSources.flatMap(([name,row])=>readSheet(workbook,name,row,"PV")).forEach((record)=>{if(record.quantity==null){invalidQuantityCount++;return;}if(record.quantity<=0){quantityZeroCount++;return;}const dedupKey=`PV|${record.reference}`;if(!unique.has(dedupKey))unique.set(dedupKey,record);});
  ...
  const now=new Date();
  byId("portDailyStart").value=monthKey(now);
  byId("portDailyEnd").value=monthKey(new Date(now.getFullYear(),now.getMonth()+1,1));
  renderAll();
}
...
function renderBusinessViews(){
  renderModelFilters();
  const allMain=activeMainRecords(),main=allMain.filter((r)=>matchesModel(r,"order")&&inMonthRange(orderMonth(r),"orderStartMonth","orderEndMonth"));
  ...
  const arrivalMain=panelMainRecords("arrival");
  const arrivals=groupRows(arrivalMain,(r)=>{if(!isArrivalValid(r))return null;const a=arrivalInfo(r),month=monthKey(a.date);return matchesModel(r,"arrival")&&month&&inMonthRange(month,"arrivalStartMonth","arrivalEndMonth")?[month,modelLabel(r),String(resolvePort(r.rawDestination,"DEST")||t("unknown")),String(r.sku||t("unknown")),a.type]:null;}).sort((a,b)=>a.parts[0].localeCompare(b.parts[0])||a.parts[1].localeCompare(b.parts[1])||a.parts[2].localeCompare(b.parts[2])||a.parts[3].localeCompare(b.parts[3])||a.parts[4].localeCompare(b.parts[4]));
  renderTable("arrivalTable",[t("hArrivalMonth"),t("hModel"),t("hDestination"),t("hSku"),t("hArrivalType"),t("hQuantity"),t("hMw"),t("hContainers")],arrivals.map((x)=>{const typeHtml=x.parts[4]==="actual"?`<span class="actual">${escapeHtml(t("actual"))}</span>`:x.parts[4]==="planned"?`<span class="planned">${escapeHtml(t("planned"))}</span>`:`<span class="forecast">${escapeHtml(t("forecast"))}</span>`;return[x.parts[0],x.parts[1],x.parts[2],x.parts[3],{html:typeHtml},fmtNumber(x.quantity),fmtNumber(x.mw,3),fmtNumber(x.containers,1)];}));
}
...
function renderRouteFilters(resetDestination=false){
  ...
  activeMainRecords().forEach((r)=>{if(!r.atd||!r.ata||!r.rawPol||!r.rawDestination)return;const lead=Math.round((r.ata-r.atd)/86400000);if(lead<=0||lead>180)return;const pol=resolvePort(r.rawPol,"POL"),destination=resolvePort(r.rawDestination,"DEST");if(pol&&destination)pairs.set(`${pol}|${destination}`,{pol,destination});});
...
function renderRoutes(){const voyages=buildVoyages(true),routes=routeStats(voyages);byId("recordKpi").textContent=fmtNumber(activeMainRecords().length);byId("voyageKpi").textContent=fmtNumber(voyages.length);byId("routeKpi").textContent=fmtNumber(routes.length);byId("zeroKpi").textContent=fmtNumber(quantityZeroCount);renderRouteTrend(voyages);renderTable("routeTable",[t("hPol"),t("hDestination"),t("hVoyages"),t("hReferences"),t("hContainers"),t("hMin"),t("hAverage"),t("hMax"),t("hP90"),t("hHistory")],routes.map((x)=>[x.pol,x.destination,x.voyages,x.references,fmtNumber(x.containers,1),x.min,x.average.toFixed(1),x.max,x.p90,`${isoDate(x.first)} ～ ${isoDate(x.last)}`]));if(hasLoadedWorkbook)statusEl.textContent=t("done",{records:fmtNumber(activeMainRecords().length),zero:quantityZeroCount,invalid:invalidQuantityCount,voyages:fmtNumber(voyages.length),mapped:skuModelMatched,main:mainRecordCount});}
...
/* ── Port daily arrival container analysis ── */
const PORT_DAILY_COLORS=["#3f8bff","#e85f3a","#2ecc8f","#f5a623","#9b59d4","#1ab6e0","#e74c8c","#3ac569"];
function availableArrivalPorts(){
  const ports=new Set();
  panelMainRecords("portDaily").forEach((r)=>{if(!isArrivalValid(r))return;const a=arrivalInfo(r);if(a.date){const dest=resolvePort(r.rawDestination,"DEST");if(dest)ports.add(dest);}});
  return[...ports].sort((a,b)=>a.localeCompare(b));
}
function buildPortDailyData(){
  const startMonth=byId("portDailyStart").value,endMonth=byId("portDailyEnd").value;
...
function renderPortDailyChart(){
  const target=byId("portDailyChart"),data=buildPortDailyData();
  const ports=[...data.keys()].sort((a,b)=>a.localeCompare(b));
  if(!ports.length){target.innerHTML=`<div class="port-daily-chart-empty">${escapeHtml(t("noData"))}</div>`;return;}
...
function renderAll(){renderBusinessViews();renderMergeReview();renderRouteFilters();renderRoutes();renderPerformance();renderMappingTable();renderPortDailyFilters();renderPortDailyChart();renderPortDailyTable();}
...
setDefaultMonthRanges();{const now=new Date();byId("portDailyStart").value=monthKey(now);byId("portDailyEnd").value=monthKey(new Date(now.getFullYear(),now.getMonth()+1,1));}renderModelFilters();renderMergeReview();renderRouteFilters();renderRouteTrend([]);renderMappingTable();renderPortDailyFilters();renderPortDailyChart();renderPortDailyTable();
