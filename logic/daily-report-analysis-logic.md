...
- PV 和 ESS ASP 纵向分别展示。每张图中上半区为 ASP 折线图（domain 52%–100%），下半区为销售数量柱状图（domain 0%–45%），共用 X 轴但上下错开不重叠。每张图都可独立按 `Region`、`Brand`、`Level1`、`Level2` 多选筛选；上层筛选变化后，下层选项仅保留当前上层筛选范围内存在的值。
